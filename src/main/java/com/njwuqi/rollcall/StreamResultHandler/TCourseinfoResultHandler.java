 ...
 */
@Component
public class TCourseinfoResultHandler implements ResultHandler<Courseinfo> {
    @Autowired
     ...
     */
    @Override
    public void handleResult(ResultContext<? extends Courseinfo> resultContext){
        // 这里获取流式查询每次返回的单条结果
        Courseinfo resultObject = resultContext.getResultObject();
        Courseinfo courseinfo = resultObject;
